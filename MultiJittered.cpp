...
MultiJittered::GenerateSamples(void) {
	...
	for (int p = 0; p < num_sets; p++) 
		for (int i = 0; i < n; i++)		
			for (int j = 0; j < n; j++) {
				samples[i * n + j + p * num_samples].X = (i * n + j) * subcell_width + MathHelper::RandFloat(0, subcell_width);
				samples[i * n + j + p * num_samples].Y = (j * n + i) * subcell_width + MathHelper::RandFloat(0, subcell_width);
			}
	...
	for (int p = 0; p < num_sets; p++) 
		for (int i = 0; i < n; i++)		
			for (int j = 0; j < n; j++) {
				int k = MathHelper::RandInt(j, n - 1);
				float t = samples[i * n + j + p * num_samples].X;
				samples[i * n + j + p * num_samples].X = samples[i * n + k + p * num_samples].X;
				samples[i * n + k + p * num_samples].X = t;
			}
